...
class CybereasonMC(object):
    ...
    def get_malops(self, params):
        ...
        '''
        start_time = arrow.get(params.get("start_time")).int_timestamp * 1000 if params.get(
            "start_time") else arrow.now().shift(minutes=-LAST_X_MINUTES).int_timestamp * 1000
        end_time = arrow.get(params.get("end_time")).int_timestamp * 1000 if params.get(
            "end_time") else arrow.now().int_timestamp * 1000
